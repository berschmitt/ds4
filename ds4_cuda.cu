...
__global__ static void moe_gate_up_mid_decode_lut_hwarp16_pair2_kernel(
        float *gate_out,
        float *up_out,
        float *mid_out,
        const char *gate_base,
    ...
    uint32_t lane = threadIdx.x & 15u;
    uint32_t row_lane = threadIdx.x >> 4u;
    uint32_t pair0 = blockIdx.y * 2u;
    uint32_t tok = pair0 / n_expert;
    const cuda_block_q8_K *xqb = xq + (uint64_t)tok * xq_blocks;
    __shared__ cuda_block_q8_K sxq[16];
    __shared__ uint64_t s_iq2_grid[256];
    __shared__ uint8_t s_iq2_signs[128];
        ...
        if (ok) {
            ...
            if (ok && sorted_pairs && use_expert_tiles && sorted_offsets && sorted_counts && tile_total && tile_experts && tile_starts) {
                ...
                if (use_decode_lut_gate && q4k_path) {
                    ...
                    if (n_tokens == 1u && getenv("DS4_CUDA_MOE_DECODE_LUT_PAIR2") != NULL) {
                        dim3 hgrid((expert_mid_dim + 63u) / 64u, (n_tokens * n_expert + 1u) / 2u, 1);
                        moe_gate_up_mid_decode_lut_hwarp16_pair2_kernel<<<hgrid, 256>>>(
                            (float *)gate->ptr,
                            (float *)up->ptr,
                            (float *)mid->ptr,
                            gate_w,
                            ...
                            write_gate_up,
                            clamp);
                    } else if (getenv("DS4_CUDA_MOE_NO_DECODE_LUT_H16") == NULL) {
