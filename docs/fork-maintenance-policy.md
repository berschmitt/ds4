...
| Single-token dense/mixed attention warp-dot probe | Historical only | May 19 A/B regressed generation badly (`43.87` / `43.83` t/s patched vs `47.25` / `47.26` baseline). Do not replay the batched warp-dot path for single-token attention. |
...
| CUB top-k 12288 extension | Candidate patch | May 19 boundary diagnosis showed the long-context generation cliffs are caused by `compressed_kv_rows` crossing exact-CUB top-k limits. Focused A/B with `gen_tokens=256` showed `10240` recovers `ctx=34816..38912`, while `12288` recovers `ctx=40960` and `43008` by about `+2.6-2.7` gen t/s and partially helps `45056..47104`. It preserves the known default `make test` failure shape. Good candidate to carry through `12288`; extend further only with a new A/B. |
...
| Broad after-prefill Q8->Q4 decode preset | Candidate opt-in preset | May 19 Shoji triage showed broad startup Q4 explains the short-context decode jump but hurts prefill. Local broad-after-prefill policy kept prefill and improved `ctx=32768`, 512-token generation (`504.67` / `49.06` vs fast-preset control `502.14` / `46.90`) while preserving the known `logprob-vectors` 7-failure shape. Keep behind env vars; do not make default until server/multi-session behavior is tested. |
...
| Other rejected top-k/indexer shortcuts | Historical only | Narrow `8704` is adopted and the `12288` staircase is a live candidate. Earlier row-order, grouped-head, tail, and chunk variants either regressed or lacked a clean comparison harness; do not replay them automatically. |
