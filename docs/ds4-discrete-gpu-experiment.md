...
1. **CUDA Graph replay feasibility**
   ...
   - Build-control run on `codex/cuda-graph-replay-prototype`, `~/ds4/codex-runs/20260516-012429-graph-stream-control`, compared normal NVCC default stream vs `CUDA_DEFAULT_STREAM=per-thread` with graphs off and `DS4_CUDA_Q8_F16_CACHE_RESERVE_MB=1024`. Generation was unchanged within noise: `ctx=4096` `44.22 -> 44.33` t/s, `ctx=32768` `38.99 -> 39.13` t/s. Prefill was slightly lower in this single run (`493.14 -> 480.77` t/s at `ctx=4096`, `445.59 -> 438.30` t/s at `ctx=32768`), so treat per-thread default stream as a graph-enabling build mode, not a standalone optimization.
   - Consolidated prototype probe on the same branch, `~/ds4/codex-runs/20260516-013241-cuda-graph-prototype-probe`, rebuilt with `CUDA_DEFAULT_STREAM=per-thread` and confirmed the opt-in probe still captures and launches with zero failures: skip-0 `12/12`, skip-4 `8/8`, skip-8 `4/4`. This keeps CUDA Graph capture/replay technically viable, but does not change the conclusion that per-token capture/instantiate/destroy is only a probe.
   - Direct `ds4-bench` A/B of that naive per-token probe, run `~/ds4/codex-runs/20260516-013444-cuda-graph-per-token-ab`, was negative: `ctx=4096` generation `44.55 -> 40.91` t/s, `ctx=32768` `39.15 -> 36.33` t/s, with `512/512` captures and launches in each graph-probe run. Close the capture-every-token path as an optimization; reusable replay is the only CUDA Graph variant worth further implementation work.
   - `cudaGraphExecUpdate` probe, run `~/ds4/codex-runs/20260516-014528-cuda-graph-update-ab`, failed after only four decode tokens: instantiated once, updated twice, launched three times, then CUDA returned `cudaErrorGraphExecUpdateFailure` because the captured graph violated update constraints. This confirms the current host decode path is not structurally stable enough for "recapture and update exec" either.
   - Static replay blockers in the current single-token decode path are concrete: per-token `token` and `pos`, raw-cache `raw_row`/`raw_start`/`n_raw`, per-layer compressed-cache counters (`layer_n_comp`, `layer_n_index_comp`), the compressor `emit` branch at ratio boundaries, optional top-k/indexed-attention branching, and the `cur_hc` / `after_ffn_hc` ping-pong after every layer. A reusable full-token graph likely needs a decode metadata buffer and fixed buffer ownership, not only `cudaGraphLaunch` around the existing host loop.
