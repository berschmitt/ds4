...
- Repeat validation closed this path as a default candidate: `~/ds4/codex-runs/20260516-024043-f16-hc-indexer-repeat` ran three control and three `hc,indexer` long-context passes. Control passed 2/3 with stable passing hash `3d7923...bb5`; `hc,indexer` passed 0/3 and repeatedly hit the Alice assignment failure. Keep shape-gated cuBLAS as opt-in instrumentation only, not a performance patch to promote.
