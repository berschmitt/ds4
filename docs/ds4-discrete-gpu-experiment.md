...
3. **CUDA Graph replay feasibility**
   ...
   - May 18 artifact review: 4-slot and 8-slot graph-update caches fail on token 128, while 128 slots survive. That strongly suggests decode topology is phase-dependent at the compressor/indexer boundary. A worthwhile graph implementation probably needs a 128-phase static graph cache and a way to move token-varying state (`token`, `pos`, raw/cache frontiers, compressor counters, buffer ping-pong) into device-side metadata or very cheap node parameter updates. Replaying the previous recapture/update-per-token prototype is not useful.
   - May 18 graph stability probe, run `~/ds4/codex-runs/20260518-234730-graph-stability-probe`: an opt-in diagnostic build with CUDA per-thread default stream captured, instantiated, and replayed 256 decode-token graphs successfully (`captured=256 launched=256 failed=0`). The API path is viable.
   - The same probe shows why naive replay is not enough. At `ctx=4096`, the 256-token run produced `128` phase slots, but every slot changed topology on its second visit (`topo_changes=128`). A follow-up run `~/ds4/codex-runs/20260518-234916-graph-stability-followup` showed a stable node-count rhythm (`1740,1740,1740,1950` kernel nodes for the first 16-token windows), but different topology hashes at positions `4096`, `4224`, `4352`, and `4608`.
   - Interpretation: CUDA Graph replay is still possible, but not as a simple fixed modulo-128 graph bank. Either the kernel sequence/function selection changes with context/frontier state, or the current graph hash is catching token/context-dependent kernel dispatch. The next graph attempt should first make the decode launch topology structurally static, then use graph replay. Do not spend more time on per-token capture/update probes.
